...
INDIR=processed
OUTDIR=tag_expr_cmp_wt
MTSA=../../bin/mtsa.py

mkdir -p $OUTDIR

# split data into two groups: one for training, the other for evaluation
cut -f 1,4- ${INDIR}/mpra_dna_counts_wt.txt >${OUTDIR}/dna_tr.txt
cut -f 1,4- ${INDIR}/mpra_mrna_counts_wt.txt >${OUTDIR}/mrna_tr.txt
cut -f 1,4- ${INDIR}/mpra_tags.txt >${OUTDIR}/tag_tr.txt
cut -f 2-3 ${INDIR}/mpra_dna_counts_wt.txt >${OUTDIR}/dna_te.txt
cut -f 2-3 ${INDIR}/mpra_mrna_counts_wt.txt >${OUTDIR}/mrna_te.txt
...
# build data
EXPNAME=${OUTDIR}/mtsa_ino17_wt_tr.m50.t10.e5
